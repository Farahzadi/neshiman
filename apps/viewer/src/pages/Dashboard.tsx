...
import { useRooms, useTeams, useUsers } from '@neshiman/api-client';
...
const Dashboard: Component = () => {
  ...
  const currentUser = createMemo(() =>
    allUsers.data?.find((u) => u.id === userId())
  );
    ...
    <div>
      <Show when={!!userId()} fallback={
        <div class="text-center py-20">
          <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center mx-auto mb-5 shadow-lg shadow-blue-200">
            <span class="text-2xl text-white font-bold">N</span>
          </div>
          <h2 class="text-xl font-bold text-gray-900 mb-2">Welcome to Neshiman</h2>
          <p class="text-gray-500 mb-8 max-w-md mx-auto">
            Select a user from the top-right dropdown to start reserving seats.
          </p>
          <div class="flex items-center justify-center gap-3">
            <For each={allUsers.data?.slice(0, 5)}>
              {(u) => (
                <button
                  onClick={() => {
                    localStorage.setItem('viewer_user_id', u.id!);
                    window.location.reload();
                  }}
                  class="px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all shadow-sm"
                >
                  {u.name}
                </button>
              )}
          ...
          <Show when={(allUsers.data?.length ?? 0) > 5}>
            <p class="text-xs text-gray-400 mt-3">+ {(allUsers.data?.length ?? 0) - 5} more users</p>
          </Show>
        </div>
      }>
        <div class="mb-8">
