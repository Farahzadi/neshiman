...
const Login: Component = () => {
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      if (role !== 'superadmin' && role !== 'team_admin') {
        setError('Admin access requires superadmin or team_admin role.');
        return;
      }
    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-100">
      <div class="bg-white rounded-2xl shadow-xl border border-gray-200 p-8 w-full max-w-sm mx-4">
        <div class="flex items-center gap-2.5 mb-8 justify-center">
          ...
          <span class="text-xl font-bold text-gray-900">Neshiman Admin</span>
        </div>
        ...
        <p class="mt-6 text-xs text-gray-400 text-center">
          Default seed: Admin / password (use any seed username)
        </p>
      </div>
    </div>
  );
