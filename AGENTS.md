...
- **Phase 7 — Soft Delete**: Migration `000008_add_deleted_at` adds `deleted_at TIMESTAMPTZ` to all 6 tables (rooms, teams, users, seats, reservations, cross_team_requests). All SELECT queries filter `AND deleted_at IS NULL`. DELETE operations became `UPDATE ... SET deleted_at = now()`. Domain entities have `DeletedAt *time.Time`. All 6 Postgres repositories use `SoftDelete*` queries. `ErrTeamHasReferences` removed (soft delete allows deletion with dependencies). Room deletion cascades soft-delete to its seats, with `RoomService.DeleteRoom` returning seat count. HTTP handlers return `200 { "deleted": true, "seats_deleted": N }` instead of 204. Admin UI has `ConfirmModal` component replacing `confirm()` dialogs with styled modals showing side-effect info. `packages/api-types` regenerated with `DeleteResponse` type. 146 tests passing.
