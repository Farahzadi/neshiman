...
- Postgres running in Docker, 11 migrations applied, `sqlc generate` done
...
- **Phase 9 — Permanent Seat Assignment (complete)**: Migration `000011_add_assigned_user_to_seats` adds `assigned_user_id` column + partial unique index. Full backend (domain, sqlc, repo, service, HTTP handlers, DTOs, 146 tests). API client hooks `useAssignSeat`/`useUnassignSeat`. Admin RoomEditor properties panel shows assign/unassign UI with user dropdown. Viewer RoomDetail shows permanent seats in purple with "Your seat"/"Assigned" labels, blocks reservations on others' permanent seats. Viewer WeeklyCalendar shows permanent seats in purple with "✦ Permanent" badge, no daily reservation needed. `SeatHandler` enriches responses with `AssignedUserName` by looking up user name via `UserService`.
